...
@history 21/06/2016, Carlos Eduardo Niemeyer Rodrigues, Proteção com o IPLicense e atualização do ProtheusDoc
/*/
User Function MSRFATHT(lAuto,cNum, lImp)
	...
	Local cLogoMail   	:= "https://i.postimg.cc/KjbLbs7J/newaco.png" // AllTrim(GetNewPar("ZZ_LOGOEXT", ""))
	Local nHdl          := 0
	Local cFile         := ""
	...
	// Query que retorna os dados do pedido de compra e os dados dos itens.  C5_ZZCDCT
	cQuery := "SELECT "
	cQuery += "		SC5.C5_NUM, SC5.C5_EMISSAO, SC5.C5_DESC1, SC5.C5_DESC2, SC5.C5_DESC3, SC5.C5_DESC4,SC5.C5_FRETE, SC5.C5_DESPESA, "
	cQuery += "		SC5.C5_TPFRETE, SC5.C5_ESPECI1, SC5.C5_VEND1, SC5.C5_MENNOTA, SC5.C5_MENPAD, SC5.C5_CONDPAG, SC5.C5_TRANSP, SC5.C5_VOLUME1,  "
	
	if SC5->(fieldPos("C5_ZZVALID")) > 0
			cQuery += "		SC5.C5_ZZVALID,  "
	endIf
	...
	Else	
		...
		Do While  cAlias->(!Eof())
			...
			endIf    
			cFile   := "" 
			cFile   := SUPERGETMV("MV_X_DIR", .T., "C:\Temp\pedido")      
			cFile   := cFile + cAlias->C5_NUM + ".html"
			cNPv 	:= cAlias->C5_NUM
			...
			If lImp
				...
				cHtml   += 'window.onload = function() { window.print(); }'
				cHtml   += '</script>'
			EndIf

			cHtml   += '<table width="100%" border="0">'
			cHtml   += '<tr>    '
			cHtml   += '	<td width="53%" height="99"><div align="left"><img src="'+cLogoMail+'" width="252" height="232"></div></td>'
			...
			cObsCtr     :=  "teste Thierry" //Posicione("ZB7",1,xFilial("ZB7") + cAlias->C5_ZZCDCT,"ZB7_MEMO")
			...
			nLinhas := MLCount(cObsCtr,150)

			For nx := 1 To nLinhas

				cTxtLinha := MemoLine(cObsCtr,150,nx)

			Next 

			cHtml   += '  <tr> '
			cHtml   += '    <td><strong><font size="2" face="Arial, Helvetica, sans-serif">Obs. Contrato:</font></strong></td> '
			cHtml   += '    <td><font size="2" face="Arial, Helvetica, sans-serif">'+Transform(cObsCtr,'@!S100')+'</font></td>'
			cHtml   += '  </tr>'
			...
			*/													
			...
			If !lImp
				oMail := IpMailObject():newIpMailObject() 

				MsAguarde({|| lOk := oMail:send(alltrim(cEmail), titulo, cHtml)}, "Aguarde", "Enviando e-mail Pedido " + ALLTRIM(cNPv) + "...")
				
				if !lOk
					MsgBox("Pedido " + ALLTRIM(cNPv) + CHR(13) + oMail:getError(), titulo, "ALERT")			   
				endIf
			 Else

			 	nHdl := FCreate(cFile) // diretório deverá existir.
		...
		if lOk
			MsgBox("E-mail enviado com sucesso", titulo, "INFO")
		endif
